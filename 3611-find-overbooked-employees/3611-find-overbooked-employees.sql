...
WITH MeetingsWithWeek AS (
    SELECT
        m.employee_id, e.employee_name, e.department,
        DATEADD(DAY, 1 - DATEPART(WEEKDAY, m.meeting_date), CAST(m.meeting_date AS DATE)) AS week_start_date, m.duration_hours
    FROM meetings  m
